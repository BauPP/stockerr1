...
function buildTestContext() {
  const repository = new InMemoryProductRepository({
    categories: [
      ...
      { id_categoria: 2, nombre: 'Snacks', estado: 'activo' },
      { id_categoria: 3, nombre: 'Descatalogados', estado: false },
    ],
    products: [
      {
        id_producto: 1,
...
test('GET /api/products pagina y filtra por nombre, categoría y código', async () => {
  ...
  const response = await request(app).get('/api/products?page=1&size=10&name=papas&category=2&code=SNACK');

  assert.equal(response.status, 200);
  assert.equal(response.body.success, true);
  assert.equal(response.body.data.total, 1);
  assert.equal(response.body.data.items.length, 1);
  assert.equal(response.body.data.items[0].id_producto, 2);
});
...
test('DELETE /api/products/:id realiza borrado lógico y lo excluye del listado', async () => {
  ...
  assert.equal(listResponse.body.data.items.some((item) => item.id_producto === 2), false);
});
